...
int main()
{
  ...
  Serial.begin(500000);
  Serial.println();
  unsigned char scalar[32], private_key[32], prefix[32];
  field_element Gx, Gy, Gt, Gz;
  generate_random_private_key(private_key);
  hash_to_scalar_and_prefix(private_key, sizeof(private_key), scalar, prefix);
  windowed_scalar_mul(&Gx, &Gy, &Gt, &Gz, scalar, PRECOMPUTED_POINTS);
  Serial.print("Private Key: {{");
  for (size_t i = 0; i < 32; i++)
  {
    Serial.print("0x");
    if (scalar[i] < 16) Serial.print('0');
    Serial.print(scalar[i], HEX);
  ...
  {
    Serial.print("0x");
    if (prefix[i] < 16) Serial.print('0');
    Serial.print(prefix[i], HEX);
    if (i < 31) Serial.print(", ");
  }
  Serial.println("}}");

  Serial.print("Public_Key Affine x-coordinate = ");
  for (int i = 0; i < 32; i++)
  {
    if (Gx.Ed[i] < 16) Serial.print("0"); // Add leading zero for single digits
    Serial.print(Gx.Ed[i], HEX);
    Serial.print(" ");
  }
  Serial.println();

  Serial.print("Public_Key Affine y-coordinate = ");
  for (int i = 0; i < 32; i++)
  {
    if (Gy.Ed[i] < 16) Serial.print("0"); // Add leading zero for single digits
    Serial.print(Gy.Ed[i], HEX);
    Serial.print(" ");
  }
  Serial.println();
}
...
void generate_random_private_key(unsigned char* private_key)
{
  uint8_t key[32];
  uint8_t nonce[32];
  randomSeed(analogRead(A0));
  for (int i = 0; i < 32; i++) { key[i] = random(0, 255); }
  for (int i = 0; i < 32; i++) { nonce[i] = random(0, 255); }
  ChaCha chaCha;
  chaCha.clear();
  chaCha.setKey(key, sizeof(key));
  chaCha.setIV(nonce, sizeof(nonce));
  chaCha.encrypt(private_key, NULL, 32);
}
void hash_to_scalar_and_prefix(const unsigned char* input, size_t inputLen, unsigned char* scalar, unsigned char* prefix)
{
  unsigned char hash[64];
  field_element temp_scalar;
  SHA512 hashEngine;
  hashEngine.reset();
  hashEngine.update(input, inputLen);
  hashEngine.finalize(hash, sizeof(hash));
