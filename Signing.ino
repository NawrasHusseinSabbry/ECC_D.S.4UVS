...
int main()
{
  ...
  field_element Gx, Gy, Gt, Gz;
  unsigned char signature[64];
  unsigned char private_key[32] = {0x37, 0x04, 0xBE, 0x00, 0x37, 0xA1, 0x75, 0x53, 0x63, 0xEF, 0x85, 0xDC, 0xD1, 0xCB, 0x37, 0xB4, 0x46, 0x50, 0x62, 0x8B, 0x56, 0x42, 0x6A, 0xDE, 0x5D, 0x5E, 0xFD, 0x51, 0x11, 0x28, 0x5A, 0x04};
  unsigned char prefix[32] = {0xC8, 0x72, 0x3D, 0xC0, 0x56, 0x50, 0xA2, 0xDB, 0x80, 0x69, 0x77, 0x23, 0xDA, 0xF6, 0x6A, 0x1A, 0xAA, 0x37, 0x6C, 0x12, 0xF7, 0x1C, 0xDF, 0x0D, 0x72, 0xCE, 0x10, 0x24, 0xC9, 0x7D, 0x2F, 0x47};
  ...
  sign_message(message, sizeof(message), prefix, private_key, &public_key_x, &public_key_y, signature);
  ...
  Serial.print("Public Nonce R-x coordinate: ");
  for (int i = 0; i < 32; i++)
  {
    if (Rx.Ed[i] < 16) Serial.print("0");
    Serial.print(Rx.Ed[i], HEX);
    Serial.print(" ");
  }
  Serial.println();

  Serial.print("Public Nonce R-y coordinate: ");
  for (int i = 0; i < 32; i++)
  {
    if (Ry.Ed[i] < 16) Serial.print("0");
    Serial.print(Ry.Ed[i], HEX);
    Serial.print(" ");
  }
  Serial.println();
    Serial.print("Public Key -x coordinate: ");
  for (int i = 0; i < 32; i++)
  {
    if (Ry.Ed[i] < 16) Serial.print("0");
  ...
  Serial.print("Public Key -y coordinate: ");
  for (int i = 0; i < 32; i++)
  {
    if (Ry.Ed[i] < 16) Serial.print("0");
...
void sign_message(const uint8_t* message, size_t message_len, const unsigned char* prefix, const unsigned char* private_key, const field_element* public_key_x, const field_element* public_key_y, unsigned char* signature)
{
  unsigned char h[64], c[64];
  field_element r, s;
  hash_to_scalar(prefix, 32, h);
  Private_Nonce_r(&r, h, message, message_len);
  Public_Nonce_R(&Rx, &Ry, &Rt, &Rz, &r);
  calculate_c(c, public_key_x, message, message_len);
  calculate_signature(signature, &r, c, private_key);
}
...
void flip_if(field_element *r, const field_element *x, unsigned char b)
{
  ...
  mask = -mask;
  for(i=0;i<32;i++) {r->Ed[i] ^= mask & (x->Ed[i] ^ r->Ed[i]);}
}
...
void hash_to_scalar(const unsigned char* prefix, size_t prefixLen, unsigned char* output)
{
  SHA512 hashEngine;
  hashEngine.reset();
  hashEngine.update(prefix, prefixLen);
  hashEngine.finalize(output, 64);
}

void Private_Nonce_r(field_element* r, const unsigned char* h, const uint8_t* message, size_t message_len)
{
  unsigned char temp[64];
  SHA512 hashEngine;
  hashEngine.reset();
  hashEngine.update(h, 64);
  hashEngine.update(message, message_len);
  hashEngine.finalize(temp, 64);
  modulo(r, temp);
  for (int i = 0; i < 7; i++){Ed25519_scalar_sub_order(r);}
}

void Public_Nonce_R(field_element* Rx, field_element* Ry, field_element* Rt, field_element* Rz, field_element* r) {windowed_scalar_mul(Rx, Ry, Rt, Rz, r->Ed, PRECOMPUTED_POINTS);}
...
void calculate_signature(unsigned char signature[64], const field_element* r, const unsigned char* c, const unsigned char* private_key)
{
  field_element c_scalar, temp_r;
  modulo(&c_scalar,c);
  cli();
  Ed_mul(signature, c_scalar.Ed, private_key);
  sei();
  memcpy(temp_r.Ed, signature, 32);
  add(&temp_r, r, &temp_r);
  memcpy(signature, temp_r.Ed, 32);
}