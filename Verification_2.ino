...
unsigned char signature[64] = {0x77, 0xD3, 0x3D, 0xF2, 0x48, 0xF5, 0x0A, 0x89, 0xE9, 0xA8, 0x22, 0xEB, 0x45, 0xF0, 0x96, 0xF4, 0xAD, 0x7A, 0xC4, 0xB4, 0x3B, 0xCB, 0xBD, 0x2B, 0xE2, 0x99, 0x3C, 0x15, 0x77, 0xDA, 0xEA, 0xA4, 0x46, 0x61, 0xF4, 0x13, 0x9A, 0x26, 0xEC, 0x22, 0xE3, 0x6A, 0xBE, 0x4F, 0x4A, 0xA2, 0x06, 0x7A, 0xCC, 0x11, 0x49, 0x9C, 0xC4, 0x53, 0x79, 0x17, 0xF1, 0x39, 0x55, 0x3E, 0xCB, 0x61, 0x25, 0x01};
uint8_t message[] = {0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x20, 0x77, 0x6F, 0x72, 0x6C, 0x64};

field_element xr, yr, tr, zr;

static const field_element PRECOMPUTED_POINTS[16][4] PROGMEM = {
...
{
  field_element public_Nonce_t, public_Nonce_z = {{1}}, public_key_t, public_key_z = {{1}}, V1x, V1y, V1t, V1z = {{1}}, V2x, V2y, V2t, V2z = {{1}}, c_scalar;
  unsigned char c[64];
  ...
  Serial.println();
  windowed_scalar_mul(&V1x, &V1y, &V1t, &V1z, signature, PRECOMPUTED_POINTS);
...
void windowed_scalar_mul(field_element *xr, field_element *yr, field_element *tr, field_element *zr, unsigned char scalar[64], const field_element precomputed[16][4])
{
  field_element rt, temp_x, temp_y,temp_t, temp_z;

  unsigned char initial_point_window = (scalar[63] >> 4);
  memcpy_P(xr->Ed, precomputed[initial_point_window-1][0].Ed, sizeof(field_element));
  memcpy_P(yr->Ed, precomputed[initial_point_window-1][1].Ed, sizeof(field_element));
  memcpy_P(tr->Ed, precomputed[initial_point_window-1][2].Ed, sizeof(field_element));
  memcpy_P(zr->Ed, precomputed[initial_point_window-1][3].Ed, sizeof(field_element));

  for (int j = 3; j >= 0; j--){double_point(xr, yr, tr, zr);}
  initial_point_window = scalar[63] & 0x0F;

  if (initial_point_window > 0)
  {
    memcpy_P(temp_x.Ed, precomputed[initial_point_window-1][0].Ed, sizeof(field_element));
    memcpy_P(temp_y.Ed, precomputed[initial_point_window-1][1].Ed, sizeof(field_element));
    memcpy_P(temp_t.Ed, precomputed[initial_point_window-1][2].Ed, sizeof(field_element));
    memcpy_P(temp_z.Ed, precomputed[initial_point_window-1][3].Ed, sizeof(field_element));
  ...
  for (int i = 62; i >= 0; i--)
  {
    ...
    {
      memcpy_P(temp_x.Ed, precomputed[upper_window_value - 1][0].Ed, sizeof(field_element));
      memcpy_P(temp_y.Ed, precomputed[upper_window_value - 1][1].Ed, sizeof(field_element));
      memcpy_P(temp_t.Ed, precomputed[upper_window_value - 1][2].Ed, sizeof(field_element));
      memcpy_P(temp_z.Ed, precomputed[upper_window_value - 1][3].Ed, sizeof(field_element));
      add_points(xr, yr, tr, zr, &temp_x, &temp_y, &temp_t, &temp_z);
    }
    ...
    if (lower_window_value > 0)
    {
    memcpy_P(temp_x.Ed, precomputed[lower_window_value - 1][0].Ed, sizeof(field_element));
    memcpy_P(temp_y.Ed, precomputed[lower_window_value - 1][1].Ed, sizeof(field_element));
    memcpy_P(temp_t.Ed, precomputed[lower_window_value - 1][2].Ed, sizeof(field_element));
    memcpy_P(temp_z.Ed, precomputed[lower_window_value - 1][3].Ed, sizeof(field_element));
    add_points(xr, yr, tr, zr, &temp_x, &temp_y, &temp_t, &temp_z);
    }
  }
  M_Inverse_Z(zr, zr);
